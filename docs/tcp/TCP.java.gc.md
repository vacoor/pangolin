...
| 分配点 | 位置 | 类型 | 频率 |
|--------|------|------|------|
| `Unpooled.buffer(ipTotalLen)` | `Tcp4Multiplexer.java:377` | 非池化 `byte[]`（~1500 B） | 每个出包 |
...
**问题**：`Tcp4Multiplexer.java:377`，每个出包执行：
...
| P1 | `buildIp4Packet()` 换池化分配 | `Tcp4Multiplexer.java:377` | 低 | 消除每包最大堆分配（~1500 B），减压效果最确定 |
