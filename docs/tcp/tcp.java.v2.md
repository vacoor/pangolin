...
| **R2 — 抽 Sender** 🚧 | 从 `TcpSock` + `TcpOutput` + `TcpRetransmitter` + `TcpSendBuffer` + `TcpAck` 的发送侧中提出 `Sender` 类,`TcpSock.sender` 引用。(R2.0 / R2.1 / R2.2 ✅;R2.3 部分字段物理下沉 ✅ `rtoBackoffShift / retransStamp / tlpHighSeq`;cwnd/ssthresh 等耦合深字段未迁) | 大 | 中高 | ★★★★★ 发送侧从 4000+ 行跨 5 文件 → 聚合一处 |
| **R3 — 抽 Receiver** ✅ | 对称做接收侧。R3.0 / R3.1 / R3.2 均完成,Receiver 物理持有 rcvNxt/rcvWnd/rcvWup/rcvPaused 四字段。 | 中 | 中 | ★★★ 收益小于 Sender(原本已经比较聚合) |
| **R4 — 拆 TcpMultiplexer** 🚧 | 一分为二:`TcpStack`(registries + policy)+ `SegmentDispatcher`(入站路由)+ `Listener` 独立。(R4.1 Listener 字段迁移 + API 封装 ✅;R4.2+ Stack/Dispatcher 拆分 ⏳) | 中 | 中 | ★★★★ TcpMultiplexer 瘦身到 <300 行 |
...
| R2 抽 Sender(R2.0-R2.2 facade + R2.3 物理下沉 18 字段,除 cwnd/ssthresh 外全部迁出)| ✅ |
| R3 抽 Receiver(R3.0-R3.1 facade + R3.2 物理下沉 rcvNxt/rcvWnd/rcvWup/rcvPaused) | ✅ |
| R4.1 抽 Listener(listenSock + synRegistry + maxSynBacklog 物理迁移 + API 封装) | ✅ |
| R4.2+ Stack / Dispatcher 拆分 | ⏳ 未开工 |
...
**当前代码形态**:
- 5 个核心对象去单例,支持多栈并存
- Sender / Receiver 作为一等公民,所有外部调用通过它们访问发送 / 接收语义
- 物理字段仍在 TcpSock,但外部调用方看不见这个事实
...
**学 gVisor 的三元分解(Endpoint + Sender + Receiver)、学 FreeBSD 的 cc/stack SPI 克制、不学 Linux 的 struct 继承;方法名 Java 化但 javadoc 保留 Linux 锚点;R0 先补单元测试网,R1 去静态 → R2 抽 Sender 是投入产出比最高的两步,每 Phase 独立 commit 便于 bisect。**
