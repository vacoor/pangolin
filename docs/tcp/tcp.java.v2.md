...
| R7.2 rtoMs 方法体迁到 Sender | ✅ |
| R7.3a packetsInFlight 方法体迁到 Sender | ✅ |
| R7.3b onAckedByCc / onTimeoutByCc 迁到 Sender | ✅ |
| R7.3c Undo / F-RTO 家族(tcpInitUndo / tcpUndoCwndReduction / tcpTryUndoRecovery / tcpTryUndoLoss / tcpProcessFrto / tcpTryUndoDsack / clearFrto)迁到 Sender | ✅ |
| R7.3d cwnd 管理家族(tcpSlowStartAfterIdleCheck / tcpCwndRestart / tcpEnterCwr 等)迁到 Sender | ✅ |
| R7.3e tcpRackUpdateReoWnd 迁到 Sender | ✅ |
| E2E 测试覆盖扩展到 **56/56 绿**(29 类) | ✅ |
...
3. `TcpSock` 经 R7.x 瘦身后仍 1420 行 — endpoint 级杂项配置(MSS / PMTU / TS options / ACK 调度细节等)仍可继续分类下沉。

### 测试护栏

56/56 E2E 全程绿(29 个测试类),其中:
...
- 时间敏感路径(2MSL / delayedAck / keepalive / probe0 / RTO backoff / RACK reoWnd)通过 `advanceTimeBy` 覆盖

---

## 十五、R7.x TcpSock 续瘦(2026-04-24)

### 目的

R2.3 已经把 37 个发送侧字段物理迁入 `Sender`;R4.2b-4 迁走了一大批 FSM 方法。但 `TcpSock` 仍有 4 块 **行为方法**依赖 Sender 的字段,只是因历史原因留在 TcpSock 上 —— 按 "Sender 是发送侧 owner" 的口径,这些方法应该物理迁入 Sender。

R7.x 就是把这批残留方法按主题分批迁入 Sender,保持 TcpSock 上的 delegate(不破坏 `TcpChannel` / `TcpSockHandler` 等外部调用)。

### 子阶段

| 阶段 | 搬迁内容 | 方法数 | 完成度 |
|---|---|---|---|
| **R7.1** | probe / keepalive / linger2 字段 + 方法迁到 Sender | 9 字段 + 16 方法 | ✅ |
| **R7.2** | `rtoMs()` 方法体迁到 Sender | 1 | ✅ |
| **R7.3a** | `packetsInFlight()` 方法体迁到 Sender | 1 | ✅ |
| **R7.3b** | `onAckedByCc / onTimeoutByCc`(CC 反馈入口)迁到 Sender | 2 | ✅ |
| **R7.3c** | Undo / F-RTO 家族迁到 Sender | 7 | ✅ |
| **R7.3d** | cwnd 管理家族迁到 Sender | 3 | ✅ |
| **R7.3e** | `tcpRackUpdateReoWnd` 迁到 Sender | 1 | ✅ |

R7.3c 细目:`tcpInitUndo / tcpUndoCwndReduction / tcpTryUndoRecovery / tcpTryUndoLoss / tcpProcessFrto / tcpTryUndoDsack / clearFrto`。

R7.3d 细目:`tcpSlowStartAfterIdleCheck / tcpCwndRestart / tcpEnterCwr`(及 priorCwnd/priorSsthresh 保有的增减路径)。

### 收益

- `TcpSock` 从 1830 行瘦到 **1420 行**(-22%)。
- `Sender` 从 814 行扩到 **1307 行**(+61%),成为发送侧**唯一**方法 owner。
- 外部调用点(TcpChannel、TcpSockHandler、测试 harness)无一行改动 —— TcpSock 上相应方法保留为 delegate。

### 测试护栏

R7.1 到 R7.3e 每个子阶段独立 commit,且在每个 commit 之后 `56/56` E2E 全绿。

---

## 十六、E2E 覆盖扩展(R4.2 后增补,2026-04-23 → 2026-04-24)

R0 基线是 24 个用例;R4.2a 的 DispatcherRoutingTest + RegistryLifecycleTest 补了 10 个;R4.2 后续又陆续补了一大批 **协议语义级** E2E 用例,用来锁住每个 RFC 要求的行为,防止 R7.x 迁移打碎。

### 新增测试类(时间顺序)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| `PawsRejectionTest` | 1 | RFC 7323 PAWS — TSVal 回退时丢包 + SACK permitted |
| `RtoBackoffTest` | 3 | 指数退避 shift 推进;cum ACK 清零;RTO ceiling |
| `FastRecoveryTest` | 3 | 3 dupack → CA_Recovery + cwnd=ssthresh+3 (NewReno inflate) + sndUna 越 highSeq 退回 CA_Open |
| `CloseStateMachineTest` | 4 | FIN_WAIT_1/2 / CLOSING / TIME_WAIT 迁移 + 2MSL |
| `ResetValidationTest` | 3 | RFC 5961 challenge ACK 三分支 |
| `SynAckRetransmitTest` | 2 | SYN 重传 backoff 与 rearm |
| `SlowStartTest` | 2 | cwnd < ssthresh 指数增长;穿越后转拥塞避免 |
| `RstInboundTest`(扩) | 3 | 窗外 / seq 匹配 / SYN_RECV → embryonic 销毁 |
| `SackBlockTest` | 4 | 单块 + 多块 + DSACK + RACK reoWndSteps 升降 |
| `TlpProbeTest` | 1 | RFC 8985 Tail Loss Probe |
| `SpuriousRtoUndoTest` | 1 | F-RTO undo(CA_Loss → CA_Open) |
| `DataWithFinTest` | 1 | payload piggyback FIN |
| `MssFragmentationTest` | 2 | >MSS payload 切片;MSS+1 剩余 1 字节段 |
| `EmbryonicResetTest` | 3 | SYN_RECV 收到 RST / bad ACK# / recoverable 重来 |
| `WindowScaleNegotiationTest` | 2 | WS 协商 + 不协商 |
| `DsackUndoTest` | 1 | DSACK 抵消重传副本 → `tcpTryUndoDsack` → CA_Open |
| `RackLostDetectionTest` | 1 | RACK 时间窗口探测丢失(真实 Thread.sleep 制造时间缝) |

### 测试覆盖现状

- **56/56 绿**,29 个测试类
- 时间敏感路径用 `EmbeddedChannel.advanceTimeBy`(调度器时钟)+ 必要时 `Thread.sleep`(System.nanoTime 真实推进,供 RACK / `TcpSegment.sentTimeUs` 使用)
- **调试产出**:`DsackUndoTest` 首版失败暴露 `tcp_collapse_retrans` 在小段场景下会把 seg0+seg1 合并成一段,改用 MSS-sized 载荷规避(与 Linux 内核默认 `TSO/GSO` 场景一致),不是 bug。
