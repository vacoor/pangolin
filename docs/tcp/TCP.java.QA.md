...
3. 对于2,3中提到的连接建立阶段和关闭阶段, Linux 协议栈中均采用了类似的处理, 校验标识位,序列号等,但是现在关闭阶段的逻辑明显只处理了理想情景下的情况
...
4. com.github.pangolin.routing.acceptor.tun.net.v2.tcp.core.TcpSegmenter 中似乎缺少了重传定时器