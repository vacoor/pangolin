...
import com.github.pangolin.routing.internal.server.ChannelFactory;
import com.github.pangolin.routing.internal.server.StandardChannelFactory;
...
@Slf4j
public class Socks5ProxyServerHandler2 extends ChannelInboundHandlerAdapter {
    ...
    public Socks5ProxyServerHandler2() {
        this(null, null, new StandardChannelFactory());
    }

    public Socks5ProxyServerHandler2(final String username, final String password) {
        this(username, password, new StandardChannelFactory());
    }

    public Socks5ProxyServerHandler2(final String username, final String password, final ChannelFactory channelFactory) {
    ...
    protected ChannelFuture connect(final ChannelHandlerContext ctx, final Socks5CommandRequest request) throws Exception {
        ...
        ctx.channel().config().setAutoRead(false);
        return channelFactory.open(new InetSocketAddress(address, port), false, ctx.channel().config().getConnectTimeoutMillis(), ctx.channel().eventLoop(), new ChannelInboundHandlerAdapter() {
            @Override
