...
public class TcpSock extends SockCommon {
    ...
    // R7.3c:tcpTryUndoRecovery / tcpTryUndoLoss / tcpProcessFrto / tcpTryUndoDsack /
    //        tcpUndoCwndReduction / tcpInitUndo / clearFrto 已物理迁到 Sender。
    //        TcpSock 保留 delegate 以免调用方改动(后续清理见 R7.3x)。
    public void clearFrto() { sender.clearFrto(); }
    public void tcpInitUndo() { sender.tcpInitUndo(); }
    public void tcpUndoCwndReduction(boolean unmarkLoss) { sender.tcpUndoCwndReduction(unmarkLoss); }
    public boolean tcpTryUndoRecovery(int tsecr) { return sender.tcpTryUndoRecovery(tsecr); }
    public boolean tcpTryUndoLoss(int tsecr) { return sender.tcpTryUndoLoss(tsecr); }
    public boolean tcpProcessFrto() { return sender.tcpProcessFrto(); }
    public boolean tcpTryUndoDsack() { return sender.tcpTryUndoDsack(); }
    ...
    // R7.3d:tcpSlowStartAfterIdleCheck / tcpCwndRestart / tcpCwndValidate /
    //        tcpCwndApplicationLimited 物理迁到 Sender,保留 delegate。
    public void tcpSlowStartAfterIdleCheck() { sender.tcpSlowStartAfterIdleCheck(); }
    public void tcpCwndValidate(boolean isCwndLimitedFlag) { sender.tcpCwndValidate(isCwndLimitedFlag); }
