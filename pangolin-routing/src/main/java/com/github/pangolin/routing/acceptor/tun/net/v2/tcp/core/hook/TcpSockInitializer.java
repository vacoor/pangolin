 ...
 *       实现方在此决定是否 / 何时 {@link SegmentDispatcher#sendSynAck} 响应。默认实现:立发。</li>
 ...
 *   <li>{@link #onRequestDestroyed} — {@link SegmentDispatcher#inet_csk_destroy_sock(TcpRequestSock)}
...
public interface TcpSockInitializer {
     ...
     *       {@link SegmentDispatcher#inet_csk_destroy_sock(TcpRequestSock)} 负责释放,
     ...
     * {@code Ipv4SegmentDispatcher#tcp_v4_syn_recv_sock} 回退 {@code tcpGroup.next()}。
    ...
    /**
     * 半连接 req 被 {@link SegmentDispatcher#inet_csk_destroy_sock(TcpRequestSock)} 销毁前触发 —
     ...
     * 不做 3WHS。
     *
     * <p>若配置中主动想拒绝监听,显式传本常量;漏配请求会在 {@code SegmentDispatcher} 构造期
