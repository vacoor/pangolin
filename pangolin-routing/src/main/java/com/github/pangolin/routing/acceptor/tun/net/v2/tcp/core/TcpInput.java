...
public class TcpInput {
    ...
    // ── errno constants used by resetIncoming ────────────────────────────────
    ...
    /**
     * Mirrors Linux {@code resetIncoming()} (tcp_input.c).
     *
     ...
     *   tcpDone(sk)                    → tcpDone(ctx, conn, closePromise)
     *   sk->sk_error_report(sk)         → ctx.fireExceptionCaught(errException(err))
     * </pre>
     *
     * <p><b>Order: tcpDone BEFORE sk_error_report</b> — Linux closes the socket first, then
     ...
     * </ul>
     *
     * @see <a href="https://github.com/torvalds/linux/blob/master/net/ipv4/tcp_input.c#L4846">resetIncoming</a>
     */
    public static void resetIncoming(ChannelHandlerContext ctx, TcpConnection conn, ChannelPromise closePromise) {
        ...
        doneWithError(ctx, conn, err, closePromise);
    }

    public static void resetIncoming(ChannelHandlerContext ctx, TcpSock sock, ChannelPromise closePromise) {
        ...
        doneWithError(ctx, sock, err, closePromise);
    }

    public static void doneWithError(ChannelHandlerContext ctx, TcpConnection conn, int err, ChannelPromise closePromise) {
        conn.skErr(err);

        // smp_wmb() — no-op: Netty EventLoop provides single-thread sequential consistency.

        // tcpDone(sk) — close first, matching Linux order.
        tcpDone(ctx, conn, closePromise);

        // sk->sk_error_report(sk) — notify pipeline AFTER tcpDone, matching Linux order.
        // After tcpDone() / doClose(): active=false, but pipeline is still intact because
        ...
        // tcpDone itself — so sk_error_report is still called after tcpDone.
    ...
    public static void doneWithError(ChannelHandlerContext ctx, TcpSock sock, int err, ChannelPromise closePromise) {
        if (sock == null || !sock.hasConnection()) {
            return;
        }
        sock.skErr(err);
        tcpDone(ctx, sock, closePromise);
        ctx.fireExceptionCaught(errException(err));
    }

    public static ChannelFuture tcpDone(ChannelHandlerContext ctx, TcpConnection conn, ChannelPromise closePromise) {
    ...
    public static ChannelFuture tcpDone(ChannelHandlerContext ctx, TcpSock sock, ChannelPromise closePromise) {
        if (sock == null || !sock.hasConnection()) {
            return null;
        }
