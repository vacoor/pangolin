 ...
 */
@Slf4j
public class TunAcceptor2 implements Acceptor {
    ...
    public TunAcceptor2(final String ifname, final InterfaceAddressEx[] bindings, final String upstream) {
        this.ifname = ifname;
...
//                        ch.pipeline().addLast(new Tcp4MultiplexHandler(dnsEngine, socketFactory));
        ...
        return b.bind(new TunAddress(ifname, bindings)).addListener(new ChannelFutureListener() {
            ...
            public void operationComplete(final ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                    TunAdapterV2 adapter = ((TunChannelV2) future.channel()).device();
...
//                    final String[] dnsServers = {"127.0.0.1", "::1"};
                    ...
                    if (adapter instanceof WindowsTunAdapterV2) {
                        final long luid = ((WindowsTunAdapterV2) adapter).luid();
                    ...
                    } else if (adapter instanceof DarwinTunAdapterV2) {
                        ...
                        log.info("Flush DNS cache");
                    } else if (adapter instanceof LinuxTunAdapterV2) {
