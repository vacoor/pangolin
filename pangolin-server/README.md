...
**access 段保活现状(详见 BUG #3):** `pangolin-common/Channels.java:53` 的 `Channels.listen(...)` 已写死 `childOption(SO_KEEPALIVE, true)`,主 server 与 Forwarder 的裸 TCP 段在 OS 层有兜底 —— 但 Linux 默认 `tcp_keepalive_time=7200s`,需运维侧调小内核参数才有意义;WS 透传 access 可挂 `WebSocketKeepaliveHandler` 但当前未挂。
...
**3. access 侧未挂应用层心跳,只有内核 keepalive 兜底**
当前已有的保活(见 §7):

- agent SERVICE bus + backhaul 都装了 `WebSocketKeepaliveHandler(60,60,60)`,server↔agent 整条 WS 链路有 PING(双向互补)
- 主 server 与 Forwarder 监听用的 `Channels.listen(...)` 已写死 `childOption(SO_KEEPALIVE, true)`,所有 access 段在 OS 层有 keepalive 兜底

**仍未覆盖的:**

- Client↔Server 的 WS 透传 access:可以挂 WS PING(精度高、跨中间设备可靠),但目前没挂 —— `Handler#handshake0` 在替换为 `WebSocketInboundRedirectHandler` 之前没有 `addBefore` keepalive
- 裸 TCP 段虽然开了 `SO_KEEPALIVE`,但 Linux 默认 `tcp_keepalive_time=7200s`,生产上**几乎等于没开**,需要运维侧调小内核参数才生效

修复方向:
- WS 透传 access:在 `Handler#handshake0` 的 `!downgrade` 分支里 `addBefore` 一个 `WebSocketKeepaliveHandler(60,60,60)`
- 裸 TCP 段:运维侧调小 `tcp_keepalive_time` / `tcp_keepalive_intvl` / `tcp_keepalive_probes`(Netty 4 不支持单连接级别配置,只能依赖内核全局参数)
...
- **server↔agent 的 SERVICE bus 与 backhaul**:当前都挂了 `WebSocketKeepaliveHandler(60,60,60)`,60s 主动发 PING,且 agent 端 `IdleStateEvent → PING` 双向互补
- **server↔agent 全程 WebSocket**(因为 BUG #8 中 agent 全 TCP 降级永不触发),心跳方案有效
- **client↔server 的 access**:
  - WS 透传:WebSocket,可以挂 WS PING(**目前未挂**)
...
**A. 给 access WS 透传链路挂 WS PING**
`WebSocketBridgeServerHandler#handshake0` 在替换为 `WebSocketInboundRedirectHandler` **之前**也插一个 `WebSocketKeepaliveHandler`(与 backhaul 一致用 60s):

```java
if (!downgrade) {
    accessCtx.pipeline().addBefore(accessCtx.name(),
        "access-keepalive", new WebSocketKeepaliveHandler(60, 60, 60));
    accessCtx.pipeline().replace(accessCtx.name(), null,
        new WebSocketInboundRedirectHandler(backhaulCtx));
    ...
}
```

必须加在 redirect handler **之前**,否则 PING 会被 `WebSocketInboundRedirectHandler#channelRead` 直接转发到对端,而不是由本地 `WebSocketServerProtocolHandler` 回 PONG。

**B. 调小内核 keepalive 参数,激活已有的 `SO_KEEPALIVE`**
...
**D. keepalive 周期参数化(可选)**
当前三处都是硬编码 60s。如果需要按部署环境(直连 / 经云 LB / 经反向代理)分别调,可抽到配置项 —— 60s 对多数公网中间链路够用,但部分云 LB 空闲超时本身就在 60s 量级,余量较紧。
